...
def test_release_readiness_summary_is_safe_and_operator_readable(monkeypatch, tmp_path):
    ...
    assert payload["progress"]["stage"] == "runtime_readiness_incomplete"
    ...
    assert set(payload["progress"]["remaining_categories"]) >= {"runtime_readiness", "production_scope", "release_artifact", "release_evidence"}
    ...
    assert {group["id"] for group in payload["blocker_groups"]} >= {"runtime_readiness", "production_scope", "release_artifact", "release_evidence"}
