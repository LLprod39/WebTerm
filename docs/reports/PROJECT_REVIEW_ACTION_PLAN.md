...
- Moved model-local helpers into `studio/model_helpers.py` and `servers/model_helpers.py`, bringing `studio/models.py` below the standard size limit; the later `servers.models` split is now complete.
...
- Split `MultiAgentEngine` into a 469-line compatibility coordinator plus focused lifecycle, planning/reporting, task-loop, LLM, and memory services in `servers/multi_agent_engine_runner.py`, `servers/multi_agent_planning.py`, `servers/multi_agent_task_runner.py`, `servers/multi_agent_llm.py`, and `servers/multi_agent_memory.py`; `servers/multi_agent_engine.py` is now below the standard architecture limit and removed from legacy baselines.
- Split the Keycloak MCP entry point into a 483-line compatibility facade plus focused admin-client core, user/client/group operation mixins, shared client support, and non-role handlers in `key_mcp_client.py`, `key_mcp_client_users.py`, `key_mcp_client_clients.py`, `key_mcp_client_groups.py`, `key_mcp_client_support.py`, and `key_mcp_handlers.py`; `key_mcp.py` is now below the standard architecture limit and removed from legacy baselines.
- Split `servers.models` into a 67-line compatibility export plus focused `servers/models_groups.py`, `servers/models_inventory.py`, `servers/models_knowledge.py`, `servers/models_monitoring.py`, `servers/models_memory.py`, and `servers/models_agents.py`; public imports still work through `servers.models`, Django model discovery still imports all classes, and `servers/models.py` is removed from legacy baselines.
- Split `SSHTerminalConsumer` into a small compatibility WebSocket class plus focused SSH terminal mixins for lifecycle, session input, AI controls, AI plan helpers, queue execution, agent orchestration, agent support, stream IO, and compatibility lookups in `servers/consumers/ssh_terminal_*.py`; `servers/consumers/ssh_terminal.py` is now below the standard architecture limit and removed from legacy baselines.
...
- Removed all remaining legacy size baselines; `[tool.architecture.legacy_baselines]` is now empty and the strict architecture guard passes without pinned large-file exceptions.
...
- Additional MultiAgentEngine coordinator verification: `python -m ruff check --select I,UP,F` passed for `servers/multi_agent_engine.py`, the new focused `servers/multi_agent_*` coordinator modules, and focused policy/multi-agent tests; WSL `.venv` pytest passed 38 focused policy, plan-executor, subagent, task-runtime, task-iteration, task-prompt, and tool-execution tests; `python scripts\check_architecture_sizes.py --strict-new` passed after removing `servers/multi_agent_engine.py` from `[tool.architecture.legacy_baselines]`.
- Additional Keycloak MCP verification: `python -m ruff check --select I,UP,F` passed for `key_mcp.py`, focused `key_mcp_client*`, handler/config/role/server/protocol/summary/tool modules, and `app/test_key_mcp.py`; `python -m py_compile` passed for the Keycloak MCP facade/client/handler modules; WSL `.venv` pytest passed `app/test_key_mcp.py` with 5 tests; `python scripts\check_architecture_sizes.py --strict-new` passed after removing `key_mcp.py` from `[tool.architecture.legacy_baselines]`.
- Additional server model split verification: `python -m ruff check --select I,UP,F` passed for `servers/models.py` and focused `servers/models_*.py` modules; WSL `manage.py check --settings=web_ui.settings.test` passed; WSL `manage.py makemigrations --check --dry-run --settings=web_ui.settings.test` reported `No changes detected`; WSL `.venv` pytest passed 11 focused server API, agent-control, and memory API tests; `python scripts\check_architecture_sizes.py --strict-new` passed after removing `servers/models.py` from `[tool.architecture.legacy_baselines]`.
- Additional SSH terminal consumer split verification: `python -m ruff check --select I,UP,F` and `python -m py_compile` passed for `servers/consumers/ssh_terminal.py` and the focused `servers/consumers/ssh_terminal_*.py` modules; WSL `manage.py check --settings=web_ui.settings.test` passed; WSL `.venv` pytest passed the focused `SSHTerminalConsumer`, Terminal AI, direct-execution, recovery, legacy-state, and manual-input tests; `python scripts\check_architecture_sizes.py --strict-new` passed after removing the last legacy baseline entry.
...
- Additional SSH terminal agent-target verification: `python -m ruff check --select I,UP,F` passed for `servers/services/terminal_agent_context.py`, `servers/consumers/ssh_terminal.py`, and `tests/test_terminal_agent_context.py`; WSL `.venv` pytest passed `tests/test_terminal_agent_context.py`; `python scripts\check_architecture_sizes.py --strict-new` passed after moving authorised agent extra-target connection setup into `terminal_agent_context.py`, reducing the then-monolithic consumer before the final mixin split.
- Additional Studio pipeline V2 test-layout verification: `python -m ruff check --select I,UP,F` passed for `tests/test_studio_pipeline_v2.py`, the split `tests/test_studio_pipeline_v2_*.py` modules, and `tests/studio_pipeline_v2_harness.py`; WSL `.venv` pytest passed 32 focused V2 pipeline executor/API/generated-graph/schedule/validation tests; `python scripts\check_architecture_sizes.py --strict-new` passed after reducing `tests/test_studio_pipeline_v2.py` to 355 lines and removing its legacy baseline.
- Additional Studio API smoke test-layout verification: `python -m ruff check --select I,UP,F` passed for `tests/test_studio_api_smoke.py`, `tests/test_studio_pipeline_assistant_smoke.py`, and `tests/studio_api_smoke_harness.py`; WSL `.venv` pytest passed 11 focused Studio API and pipeline-assistant smoke tests; `python scripts\check_architecture_sizes.py --strict-new` passed after reducing `tests/test_studio_api_smoke.py` to 412 lines and removing its legacy baseline.
- Additional Studio executor node test-layout verification: `python -m ruff check --select I,UP,F` passed for `tests/test_studio_node_executors.py`, `tests/studio_node_executor_harness.py`, `tests/test_studio_logic_node_executors.py`, `tests/test_studio_interaction_node_executors.py`, and `tests/test_studio_agent_integration_node_executors.py`; WSL `.venv` pytest passed 24 focused executor node tests; `python scripts\check_architecture_sizes.py --strict-new` passed after reducing `tests/test_studio_node_executors.py` to 177 lines and removing its legacy baseline.
- Additional server API smoke test-layout verification: `python -m ruff check --select I,UP,F` passed for `tests/test_servers_api_smoke.py`, `tests/test_servers_memory_api_smoke.py`, `tests/test_servers_monitoring_watcher_api.py`, `tests/test_servers_agent_run_api_smoke.py`, and `tests/servers_api_smoke_harness.py`; WSL `.venv` pytest passed 16 focused server API smoke tests; `python scripts\check_architecture_sizes.py --strict-new` passed after reducing `tests/test_servers_api_smoke.py` to 267 lines and removing its legacy baseline.
- Additional ops-agent memory test-layout verification: `python -m ruff check --select I,UP,F` passed for `tests/test_ops_agent_kernel.py`, `tests/test_ops_agent_memory_store_core.py`, `tests/test_ops_agent_memory_patterns.py`, and `tests/test_ops_agent_memory_lifecycle.py`; WSL `.venv` pytest passed 42 focused ops-agent kernel/memory tests; `python scripts\check_architecture_sizes.py --strict-new` passed after reducing `tests/test_ops_agent_kernel.py` to 307 lines and removing its legacy baseline.
- Additional Keycloak provisioning graph verification: `python -m ruff check --select I,UP,F` passed for `studio/keycloak_provisioning.py`, the focused `studio/keycloak_*` modules, generated-graph tests, and setup tests; WSL `.venv` pytest passed `app/test_keycloak_provisioning_setup.py` and `tests/test_studio_pipeline_v2_generated_graphs.py` with 8 tests; `python scripts\check_architecture_sizes.py --strict-new` passed after reducing `studio/keycloak_provisioning.py` to a 113-line facade and removing its legacy baseline.
- Additional pipeline assistant service verification: `python -m ruff check --select I,UP,F` passed for `studio/services/pipeline_assistant.py`, the focused `studio/services/pipeline_assistant_*` modules, assistant API tests, template assistant tests, and assistant smoke tests; WSL `.venv` pytest passed 16 focused assistant/draft/capability tests; `python scripts\check_architecture_sizes.py --strict-new` passed after reducing `studio/services/pipeline_assistant.py` to 191 lines and removing its legacy baseline.
- Additional AgentEngine service split verification: `python -m ruff check --select I,UP,F` passed for `servers/agent_engine.py`, `servers/agent_engine_runner.py`, `servers/agent_engine_tools.py`, and focused policy/control/ops tests; WSL `.venv` pytest passed 16 focused AgentEngine policy, runtime-control, and ops prompt tests; `python scripts\check_architecture_sizes.py --strict-new` passed after reducing `servers/agent_engine.py` to 409 lines and removing its legacy baseline.
- Additional MARS service split verification: `python -m ruff check --select I,UP,F` passed for `mars/services.py`, `mars/runtime_cli.py`, `mars/interview_questions.py`, `mars/interview_codex.py`, `mars/worker.py`, and `mars/worker_phases.py`; WSL `.venv` pytest passed `tests/test_mars_api.py` and `tests/test_mars_worker.py` with 15 tests; `python scripts\check_architecture_sizes.py --strict-new` passed after reducing `mars/services.py` to 308 lines and removing its legacy baseline.
...
## Completed: SSH Terminal Consumer Split
...
- `SSHTerminalConsumer` remains the Channels-compatible public class and keeps historical private methods available through mixin inheritance.
- Connection lifecycle, manual input/session operations, AI control handlers, plan/report helpers, queue execution, agent orchestration/support, and SSH stream IO now live in focused `servers/consumers/ssh_terminal_*.py` modules.
- Marker filtering, AI exit-future resolution, bounded output buffers, manual input orchestration, direct execution, parallel batches, agent-target connection setup, and Terminal AI state transitions remain delegated to focused service modules.
- The legacy size baseline for `servers/consumers/ssh_terminal.py` has been removed; no size baseline entries remain in `pyproject.toml`.
...
| Keycloak MCP helper extraction | Done. Declarative MCP tool schemas live in `key_mcp_tools.py`, JSON-RPC response helpers live in `key_mcp_protocol.py`, summary helpers live in `key_mcp_summaries.py`, config/profile helpers live in `key_mcp_config.py`, MCP transport helpers live in `key_mcp_server.py`, role-management helpers live in `key_mcp_roles.py`, admin-client operations live in `key_mcp_client*.py`, and non-role handlers live in `key_mcp_handlers.py`; `key_mcp.py` is below the standard architecture limit and no longer appears in `[tool.architecture.legacy_baselines]`. |
| SSH terminal consumer split | Done. `servers/consumers/ssh_terminal.py` is now a small compatibility WebSocket class; lifecycle, session input, AI controls, plan helpers, queue execution, agent orchestration/support, stream IO, and compatibility lookup behavior live in focused `servers/consumers/ssh_terminal_*.py` modules. |
