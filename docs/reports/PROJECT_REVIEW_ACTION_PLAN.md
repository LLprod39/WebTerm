...
> Current-state warning (2026-07-22): this file is a historical progress snapshot. Its architecture PASS statements are no longer true for commit `b8924ee`; the live guard reports 28 size violations and a broken `core_ui -> servers` boundary. Use [the WebTerm/RoutineOps competitive plan](../architecture/WEBTERM_ROUTINEOPS_COMPETITIVE_PLAN.md) as the current stabilization roadmap.

This was the implementation backlog produced by the 2026-06-21 refresh. It replaced the older 2026-05-19 action list but is no longer the current source of truth.
...
- Moved provider model-list refresh into `app/core/model_refresh.py`; `ModelManager.fetch_available_*` methods remain compatibility delegates while Gemini, Grok, Claude, OpenAI, and Ollama local/cloud refresh logic lives outside `model_config.py`.
...
- Moved OpenAI-compatible streaming into `app/core/llm_openai_compatible.py`: OpenAI and Grok now share request construction, Responses/Completions/Chat Completions SSE parsing, retry/backoff, HTTP/timeout error handling, optional `trust_env`, and usage logging.
