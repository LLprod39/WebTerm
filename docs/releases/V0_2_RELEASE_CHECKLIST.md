...
Status: release candidate until the protected `test` commit, `v0.2.0` tag workflow, published-image smoke and signed assets are all verified.
...
1. Tag `v0.2.0` must resolve to the exact protected `test` candidate and match `VERSION`.
...
6. Only then may GitHub Release `v0.2.0` be marked latest.
