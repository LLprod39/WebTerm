...
| Backend tests | `python -m pytest --junitxml=.ci-artifacts/backend-junit.xml` | `backend-junit.xml`, zero failures/errors | Current release worktree: 2,349 passed, 3 environment-gated integration skips; clean RC/CI proof pending |
...
| Playbook durable worker | `python manage.py run_playbook_execution_plane --once --worker-key release-check` plus restart/cancel/lease tests | queue claim, heartbeat, terminal cleanup and no-auto-replay evidence | Implemented; dispatch/cutover/restart/cancel/lease tests green in the release worktree; clean RC/CI proof pending |
| Playbook isolated runtime | controller-policy exploit tests, `tests/test_ansible_docker_runtime.py`, `tests/test_ansible_validator_server.py`, and Compose validator/runner smoke | immutable image/runtime digest parity, exact claim labels and daemon cleanup, bounded networkless validator without supplementary root groups, crash-artifact scavenging, hardened per-run container and strict host keys | Image builds and starts with the pinned runtime; validator/runtime tests green in the release worktree; clean Linux RC/CI proof pending |
| Playbook private bundle storage | Set `PLAYBOOK_BUNDLE_STORAGE_ROOT` to a dedicated shared volume outside `MEDIA_ROOT`; mount it only into backend and playbook workers; prove `/media/` cannot retrieve a stored bundle | inaccessible HTTP probe plus backend/worker read-write smoke and backup evidence | Production Compose uses a dedicated `playbook_bundles` volume mounted only into backend and playbook workers; nginx deny rule, deploy check and storage-boundary tests are green locally; clean RC/CI and recovery proof pending |
