...
- Staff release readiness summary now has a read-only endpoint at `GET /api/kubernetes/release/summary/`: it does not run live provider checks, reads current readiness plus the latest release evidence artifact, groups production blockers into runtime readiness, production scope, release artifact and release evidence buckets, returns required commands/next steps for the Settings UI, returns `progress` with backend DoD percent, runtime-readiness percent, production/sidebar stage and remaining blocker categories, returns `completion_audit` with explicit `core_backend_complete`, `runtime_readiness_complete`, `production_evidence_complete`, `sidebar_enablement_complete` and remaining categories, returns `production_evidence_checklist` with required/present/status for core refs, latest external evidence bundle artifact checks and machine-readable `gap_summary`, returns `operator_command_plan` with a separate local demo smoke phase, production prerequisite/release artifact command phases, recommended next manual/command step and `blocking_summary`, returns `production_execution_plan` with the same blocked-until/phase/command contract as the handoff artifact, and exposes only metadata/status fields so raw release artifact provider tokens, credentialed URLs or env values are not serialized.
- Release handoff JSON/Markdown now preserves the same `operator_command_plan`, including the local demo smoke commands (`k8s-provider-fixture` + `seed_kubernetes_ops_demo --admin-write`) and the production prerequisite/release artifact phases, so an operator handoff can bootstrap local testing without losing the production gate sequence.
- The release summary, saved release evidence artifact, release handoff and release management command output now return `backend_workstream`: a machine-readable split between backend-scope completion and external production/sidebar blockers. It reports `backend_complete`, core backend proof counts/percent, runtime-readiness completion, remaining backend gaps, production evidence blockers, `external_production_blocker_summary` categories, safe frontend continuation flag and the next backend/prod-evidence step, so operators do not confuse local backend completeness with production release approval. The handoff sanitizer and markdown renderer preserve this category summary, while `verify_kubernetes_ops_release` and `render_kubernetes_ops_release_handoff` print the primary external blocker category for quick CLI triage.
- Local/demo backend bootstrap is now repeatable with `python manage.py seed_kubernetes_ops_demo --username admin`: it grants `kubernetes` + `kubernetes_admin_read` to an existing user by default, supports explicit local admin-demo access with `--admin-write`, seeds a safe local cluster/apps/workloads/pods/network/Fleet/events inventory backed by the local provider fixture at `http://127.0.0.1:18090`, marks the Kubernetes sync worker as a 24-hour `local-demo` heartbeat, stores no provider secrets, makes no external provider calls, and is covered by idempotency/API overview/admin-write/release-readiness regression tests. Release progress now keeps production/sidebar gates out of the runtime-readiness blocker bucket, so a local demo with fresh seeded data reports `core_backend_ready_production_blocked` instead of a false backend runtime gap.
...
- no exec after session expiry; [done: active-state gate plus provider-stream WebSocket regression closes with `admin_session_expired` and closes the provider handle]
