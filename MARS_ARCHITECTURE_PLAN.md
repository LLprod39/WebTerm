...
Базовый контекст проекта: `AGENTS.md`, `ARCHITECTURE_CONTRACT.md`, `.importlinter`, `pyproject.toml`, текущая структура `app/`, `core_ui/`, `servers/`, `studio/`, `web_ui/`, `ai-server-terminal-main/`, `desktop/`.
...
4. `ai-server-terminal-main/src/api/` уже создан как целевая точка импорта, но почти все модули пока являются re-export shim поверх `src/lib/api.ts`.
5. `web_ui/settings.py` уже разложен в пакет `web_ui/settings/*`.
6. Есть реальные architecture fitness guards: `.importlinter`, `ARCHITECTURE_CONTRACT.md`, `scripts/check_architecture_sizes.py`, baselines в `pyproject.toml`.
...
| Файл | Строк | Проблема |
| --- | ---: | --- |
| `ai-server-terminal-main/src/pages/PipelineEditorPage.tsx` | 4260 | route + graph editor + forms + assistant + run monitor |
| `ai-server-terminal-main/src/lib/api.ts` | 4254 | все DTO, fetch, WebSocket, demo fallback и endpoints в одном файле |
| `ai-server-terminal-main/src/components/terminal/LinuxUiPanel.tsx` | 4156 | desktop shell, apps, window state, API orchestration |
...
| `ai-server-terminal-main/src/pages/Servers.tsx` | 3448 | server CRUD, groups, knowledge, memory UI |
...
| `ai-server-terminal-main/src/pages/SettingsPage.tsx` | 2042 | settings container still too broad |

### 4.3 Что уже хорошо сделано

Есть реальные архитектурные зацепки, которые надо продолжать, а не ломать:

- `ARCHITECTURE_CONTRACT.md` описывает file-size и import-boundary правила.
...
- `ai-server-terminal-main/src/components/terminal/ServerPicker.tsx` вынесен из `TerminalPage.tsx`; `TerminalPage.tsx` уменьшен до 1309 строк и больше не нарушает legacy baseline.
- `ai-server-terminal-main/src/components/terminal/SftpTransferQueue.tsx` вынесен из `SftpPanel.tsx`; `SftpPanel.tsx` уменьшен до 840 строк и больше не нарушает legacy baseline.
...
- `ai-server-terminal-main/src/api/` уже создан как целевая API-структура.
- `ai-server-terminal-main/src/pages/settings/` уже содержит отдельные settings pages.
...
- `ai-server-terminal-main/src/api/index.ts` broad re-export.
