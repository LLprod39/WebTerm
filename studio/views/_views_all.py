...
def _run_queryset_for_user(user):
    qs = PipelineRun.objects.select_related("pipeline", "pipeline__owner", "triggered_by", "trigger")
    if _is_admin(user):
...
def _assistant_patch_summary(response: dict) -> str:
...
def _apply_pipeline_assistant_patch(nodes: list, edges: list, response: dict) -> tuple[list[dict], list[dict]]:
...
def _pipeline_assistant_risk(nodes: list) -> dict:
    items = []
...
def api_pipeline_assistant(request):
    ...
    graph_node_summaries = [_compact_node_summary(item) for item in node_map.values()]
    graph_overview = {
    ...
    preview_nodes, preview_edges = _apply_pipeline_assistant_patch(nodes, edges, response)
    validation_errors = validate_pipeline_definition(
        nodes=preview_nodes,
        edges=preview_edges,
    ...
    response["patch_summary"] = response.get("patch_summary") or _assistant_patch_summary(response)
    response["validation"] = {
        "ok": not validation_errors,
        "errors": validation_errors,
        "warnings": [],
    }
    response["risk"] = _pipeline_assistant_risk(preview_nodes)
