...
- Для сервисных задач (Keycloak, Kubernetes, GitLab/GitHub, базы данных, SaaS) НЕ придумывай отдельные service-specific node types. Используй минимальный универсальный набор: agent/mcp_call для действий через MCP, skills как runbook/policy, logic/human_approval перед изменениями, output/report для результата.
...
- Для Keycloak/IAM задач по умолчанию строй workflow: trigger/manual → agent/mcp_call preflight/read → logic/human_approval → agent/mcp_call change → agent/mcp_call verify или output/report. Не используй ops/* для Keycloak, если действие выполняется через MCP API.
