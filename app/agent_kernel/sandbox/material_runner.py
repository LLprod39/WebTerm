...
def build_material_runner_docker_command(*, runner_id: str = "") -> list[str]:
    ...
    return [
        ...
        "never",
        "--name",
        f"webterm-material-{resolved_id}",
        "--network",
        network,
        "--user",
        "10001:10001",
        ...
        "--pids-limit",
        str(max(8, int(getattr(settings, "AGENT_MATERIAL_RUNNER_PIDS_LIMIT", 32) or 32))),
        "--cpus",
        str(getattr(settings, "AGENT_MATERIAL_RUNNER_CPUS", "0.25") or "0.25"),
        "--memory",
        str(getattr(settings, "AGENT_MATERIAL_RUNNER_MEMORY", "128m") or "128m"),
        "--tmpfs",
        "/work:rw,exec,nosuid,nodev,size=16m,uid=10001,gid=10001,mode=700",
        "--env",
        "HOME=/work",
        "--env",
        "TMPDIR=/work",
...
async def execute_isolated_material(
    *, content: str, args: list[str], timeout_seconds: int = 120, language: str = "bash"
) -> MaterialRunResult:
    if language not in {"bash", "shell", "sh"}:
        raise MaterialRunnerError(f"Unsupported material language: {language}.")
    encoded = json.dumps(
        {
            "schema": "webterm.material-run.v1",
            "language": language,
            "content": content,
            "args": args,
            "timeout_seconds": max(1, min(int(timeout_seconds), 300)),
            "output_limit": max(
                1024, min(int(getattr(settings, "AGENT_MATERIAL_RUNNER_OUTPUT_MAX_CHARS", 50_000) or 50_000), 200_000)
            ),
        },
        ensure_ascii=False,
    ).encode("utf-8")
    input_limit = max(
        1024, min(int(getattr(settings, "AGENT_MATERIAL_RUNNER_INPUT_MAX_BYTES", 64_000) or 64_000), 1_048_576)
    )
    ...
    try:
        stdout, stderr = await asyncio.wait_for(
            process.communicate(encoded), timeout=max(1, min(int(timeout_seconds), 300)) + 10
        )
    ...
    return MaterialRunResult(
        str(payload.get("stdout") or ""),
        str(payload.get("stderr") or ""),
        int(payload.get("exit_status", -1)),
        int(payload.get("duration_ms") or ((time.monotonic() - started) * 1000)),
    )
