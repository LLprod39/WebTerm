...
export function ContentPanel({
  ...
  return (
    <section data-ui-slot="content-panel" className={cn("rounded-lg border border-border/80 bg-card/95", className)}>
      {children}
    </section>
  );
}
...
export function ContentSection({
  ...
  return (
    <section data-ui-slot="content-section" className={cn("border-t border-border/70 first:border-t-0", className)}>
      {(title || description || actions) ? (
        <div data-ui-slot="content-section-header" className="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-start sm:justify-between">
