  ...
  return (
    <div className="mb-4 rounded-lg border border-border/70 bg-background/45 px-4 py-3">
      <div className="flex flex-wrap items-center gap-2">
        <StatusBadge label={localize(lang, "Ownership summary", "Ownership summary")} tone="info" />
        <span className="text-xs text-muted-foreground">
          {localize(lang, "Всего:", "Total:")} {summary.total}
        </span>
        <span className="text-xs text-muted-foreground">
          {localize(lang, "Guarded:", "Guarded:")} {summary.guarded_items}
        </span>
      </div>
      <div className="mt-2 flex flex-wrap gap-2">
        {Object.entries(summary.owners).map(([owner, count]) => (
          <StatusBadge key={owner} label={`${ownerLabel(owner)} ${count}`} tone={ownerTone(owner)} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div className="mb-4 rounded-lg border border-border/70 bg-background/45 px-4 py-3">
      ...
      <div className="mt-3 grid gap-3 text-xs text-muted-foreground md:grid-cols-3">
        ...
        <div>
          <div className="font-medium text-foreground">{localize(lang, "Evidence", "Evidence")}</div>
...
export function ownerLabel(owner: string) {
...
function changePathText(lang: string, changePath: string) {
  ...
  if (changePath === "fleet_gitops_or_mr") return localize(lang, "Менять через GitOps/Fleet rollout в WebTerm.", "Use the GitOps/Fleet rollout flow in WebTerm.");
  ...
  return localize(lang, "Только read-only Admin session; write ещё выключен.", "Read-only Admin session only; write is still disabled.");
}
