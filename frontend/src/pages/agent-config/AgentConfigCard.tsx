    ...
    <div className="group overflow-hidden rounded-xl border border-border bg-card shadow-sm transition-all duration-150 hover:shadow-md">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex shrink-0 gap-1">
            <Button
              ...
              aria-label={
                agent.can_edit === false
                  ? localize(lang, `Открыть профиль ${agent.name}`, `View profile ${agent.name}`)
                  : localize(lang, `Изменить профиль ${agent.name}`, `Edit profile ${agent.name}`)
              }
              title={agent.can_edit === false ? localize(lang, "Открыть профиль", "View profile") : localize(lang, "Изменить профиль", "Edit profile")}
            >
      ...
      <div className="space-y-2.5 border-t border-border/50 bg-secondary/10 px-4 py-3">
        <div className="flex flex-wrap gap-1.5">
          <span className="rounded border border-border/50 bg-secondary/40 px-1.5 py-0.5 font-mono text-xs text-muted-foreground">
            {agent.model}
          </span>
          <span className="rounded border border-border/50 bg-secondary/40 px-1.5 py-0.5 text-xs text-muted-foreground">
        ...
        {agent.skill_errors?.length ? (
          <div className="rounded-lg border border-amber-500/25 bg-amber-500/8 px-2.5 py-1.5">
            {agent.skill_errors.slice(0, 1).map((error) => (
              <p key={error} className="text-xs text-amber-300">
                {error}
              </p>
            ))}
          </div>
