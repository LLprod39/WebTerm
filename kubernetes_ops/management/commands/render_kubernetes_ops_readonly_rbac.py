...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--namespace",
            default=READONLY_SERVICE_ACCOUNT_CONTRACT["namespace"],
        ...
        parser.add_argument(
            "--validate-only", action="store_true", help="Validate the manifest and print no YAML/JSON payload."
        )
    ...
    def handle(self, *args, **options):
        ...
        payload = (
            render_kubernetes_readonly_rbac_json(bundle)
            if options["format"] == "json"
            else render_kubernetes_readonly_rbac_yaml(bundle)
        )
