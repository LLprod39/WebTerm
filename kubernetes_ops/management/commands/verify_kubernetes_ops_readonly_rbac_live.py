...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--manifest", default="artifacts/kubernetes_ops_readonly_rbac.yaml", help="Rendered read-only RBAC manifest path.")
        parser.add_argument("--output", default="artifacts/kubernetes_ops_readonly_rbac_live_evidence.json", help="Output JSON evidence path.")
        ...
        parser.add_argument("--namespace", default=READONLY_SERVICE_ACCOUNT_CONTRACT["namespace"], help="ServiceAccount namespace.")
        parser.add_argument("--service-account", default=READONLY_SERVICE_ACCOUNT_CONTRACT["name"], help="ServiceAccount name.")
        parser.add_argument("--probe-namespace", default="default", help="Namespace used for namespaced can-i probes.")
        parser.add_argument("--no-fail", action="store_true", help="Return exit code 0 even when live proof is not ready.")
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(json.dumps({"status": report["status"], "context": report["context"], "errors": report["errors"]}, ensure_ascii=False))
