...
def build_restricted_kube_context_for_session(
    *, session: K8sAdminSession, include_manifest: bool = False
) -> dict[str, Any]:
...
def _validate_session(session: K8sAdminSession) -> None:
    ...
    if session.mode != K8sAdminSession.MODE_BREAK_GLASS:
        raise AdminResourceError(
            "Restricted kube context requires a break-glass admin session.",
            code="break_glass_session_required",
            status=403,
        )
    if session.cluster_id is None:
        raise AdminResourceError(
            "Restricted kube context requires a scoped cluster.", code="admin_session_cluster_required", status=403
        )
...
def _single_namespace(session: K8sAdminSession) -> str:
    ...
    if not namespace or namespace == "*" or "*" in scoped or len(scoped) != 1 or namespace not in scoped:
        raise AdminResourceError(
            "Restricted kube context requires exactly one namespace.",
            code="restricted_context_namespace_required",
            status=403,
        )
    if namespace in set(DEFAULT_PROTECTED_NAMESPACES):
        raise AdminResourceError(
            "Restricted kube context is blocked in protected namespaces.",
            code="restricted_context_namespace_protected",
            status=403,
        )
    return namespace
...
def _rules_for_session(session: K8sAdminSession) -> list[dict[str, Any]]:
    ...
    if len(rules) == len(BASE_READ_RULES):
        raise AdminResourceError(
            "Restricted kube context requires exec or port-forward in the session scope.",
            code="restricted_context_terminal_verbs_required",
            status=403,
        )
    return rules
    ...
    return [
        {
            "apiVersion": "v1",
            "kind": "ServiceAccount",
            "metadata": {"name": service_account_name, "namespace": namespace, **deepcopy(metadata)},
        },
        {
            "apiVersion": "rbac.authorization.k8s.io/v1",
            "kind": "Role",
            "metadata": {"name": role_name, "namespace": namespace, **deepcopy(metadata)},
            "rules": deepcopy(rules),
        },
        {
            "apiVersion": "rbac.authorization.k8s.io/v1",
...
def _validate_rules(rules: list[dict[str, Any]]) -> list[str]:
    ...
    for rule in rules:
        ...
        illegal_base_writes = {
            verb for verb in verbs & DENIED_BASE_WRITE_VERBS if not resources <= {"pods/exec", "pods/portforward"}
        }
...
def _role_binding_matches(bundle: dict[str, Any]) -> bool:
    bindings = [
        item for item in bundle.get("manifests", []) if isinstance(item, dict) and item.get("kind") == "RoleBinding"
    ]
    for binding in bindings:
        ...
        if any(
            ...
            and item.get("name") == bundle.get("service_account_name")
            and item.get("namespace") == bundle.get("namespace")
