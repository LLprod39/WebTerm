    ...
    *,
    ...
    if not bool(getattr(settings, "KUBERNETES_ADMIN_NATIVE_PATCH_ENABLED", False)):
        raise AdminResourceError(
            "Native Kubernetes patch is disabled by policy.", code="native_patch_disabled", status=403
        )
...
def _active_patch_session_for_user(
...
def _patch_request_summary(ref: KubernetesResourceRef, *, body: Any, patch_type: str, reason: str) -> dict[str, Any]:
    if isinstance(body, dict):
        shape = {
            "body_shape": "object",
            "top_level_fields": sorted(str(key)[:120] for key in body),
        }
    else:
        ops = []
        for item in body[:50]:
...
def _required_rancher_provider(cluster: K8sCluster) -> K8sProvider:
    ...
    if provider is None or not provider.enabled:
        raise AdminResourceError(
            "Enabled Rancher provider is required for Admin Mode patch.", code="rancher_provider_required", status=409
        )
    return provider
