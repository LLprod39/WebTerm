        ...
        plan.update(
            {
                ...
                "steps": ["verify current health", "scale workload back to previous_replicas if rollback is required", "verify workload readiness"],
        ...
        plan.update(
            {
                ...
                "steps": ["verify rollout health", "use GitOps/Devtron previous version rollback if restart exposes a bad release", "verify pods are ready"],
        ...
        plan.update(
            {
                ...
                "steps": ["prepare previous manifest or GitOps revert", "run fresh dry-run apply for rollback manifest", "apply rollback only after approval"],
                "evidence_required": ["rollback_source_ref", "rollback_dry_run_action_id", "post_rollback_resource_generation"],
            }
        )
        ...
        plan.update(
            {
                ...
                "steps": ["capture current sanitized resource state", "prepare reverse patch or GitOps revert", "dry-run and approve rollback patch"],
                "evidence_required": ["previous_resource_snapshot_ref", "reverse_patch_dry_run_ref", "post_rollback_resource_generation"],
            }
        )
        ...
        plan.update(
            {
                ...
                "steps": ["confirm restore source exists before deletion", "restore via GitOps or approved apply", "verify dependent workload health"],
