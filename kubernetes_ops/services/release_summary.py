...
def build_kubernetes_release_summary(evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    normal_user_surface = evidence.get("normal_user_surface") if isinstance(evidence.get("normal_user_surface"), dict) else {}
    definition_of_done = evidence.get("definition_of_done") if isinstance(evidence.get("definition_of_done"), dict) else {}
    frontend_scan = (
    ...
    secret_read_controls = evidence.get("secret_read_controls") if isinstance(evidence.get("secret_read_controls"), dict) else {}
    provider_secret_lifecycle = evidence.get("provider_secret_lifecycle") if isinstance(evidence.get("provider_secret_lifecycle"), dict) else {}
    audit_redaction = evidence.get("audit_redaction") if isinstance(evidence.get("audit_redaction"), dict) else {}
    production_action_evidence = evidence.get("production_action_evidence") if isinstance(evidence.get("production_action_evidence"), dict) else {}
    ...
    summary = {
        ...
        "production_action_blocked_action_class_count": int(production_action_summary.get("blocked_action_class_count") or 0),
...
def _release_next_steps(blockers: list[str]) -> list[str]:
    ...
    if any(item.startswith("release_scope:") for item in blockers):
        steps.append("Run release evidence in production with non-local Rancher/Devtron/MCP endpoints, approval ref and core evidence refs.")
    if any(item.startswith("production_action_evidence:") for item in blockers):
        steps.append("Regenerate production action evidence and fix rollback/native verification/blocked-action contracts.")
    ...
    if any(item.startswith("interactive_transport_evidence:") for item in blockers):
        steps.append("Run verify_kubernetes_ops_interactive_transport_evidence and fix recording, credential, network-policy or provider-contract prerequisites.")
    if any(item.startswith("interactive_live_smoke:") for item in blockers):
        steps.append("Run verify_kubernetes_ops_interactive_live_smoke and provide production live-smoke evidence refs before enabling interactive streams.")
    ...
    if any(item.startswith("normal_user_surface:") for item in blockers):
        steps.append("Fix WebTerm-only normal-user API proof: hide external links/provider config and keep fallback deeplink audit staff-only.")
