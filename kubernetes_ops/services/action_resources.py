...
def resource_apply_preview(
    target: dict[str, Any], *, user, summary: str
    ...
    if not dry_run_action_id:
        raise ActionRequestValidationError(
            "dry_run_action_id is required for apply requests.", code="dry_run_proof_required"
        )
    try:
        ...
        raise ActionRequestValidationError(
            "A successful dry-run proof is required for apply requests.", code="dry_run_proof_required"
        ) from exc
    if proof.user_id != getattr(user, "id", None):
        raise ActionRequestValidationError(
            "Dry-run proof must belong to the requester.", code="dry_run_proof_owner_mismatch"
        )
    ...
    if requested_cluster is not None and requested_cluster.id != cluster.id:
        raise ActionRequestValidationError(
            "Dry-run proof target does not match requested cluster.", code="dry_run_target_mismatch"
        )
...
def resource_patch_preview(
    ...
    if not ref.namespace or not ref.name:
        raise ActionRequestValidationError(
            "patch requires namespace, kind and name.", code="resource_target_required", payload={"target": target}
        )
    ...
    return (
        ...
        normalized,
        {
            ...
            "expected_verification": [
                "resource generation observed",
                "workload readiness if applicable",
                "recent warning events",
            ],
        },
    )
...
def resource_delete_preview(
    ...
    if not ref.namespace or not ref.name:
        raise ActionRequestValidationError(
            "delete requires namespace, kind and name.", code="resource_target_required", payload={"target": target}
        )
    if ref.kind in PROTECTED_CLUSTER_KINDS:
        raise ActionRequestValidationError(
            "This resource kind cannot be deleted through action requests.",
            code="delete_kind_blocked",
            payload={"kind": ref.kind},
        )
    if ref.namespace in protected_delete_namespaces():
        raise ActionRequestValidationError(
            "Deletes in protected namespaces are blocked.",
            code="delete_namespace_protected",
            payload={"namespace": ref.namespace},
        )
...
def _clean_safe_patch_body(target: dict[str, Any]) -> tuple[str, Any, dict[str, Any]]:
    ...
    if patch_type not in PATCH_TYPES:
        raise ActionRequestValidationError(
            "patch_type must be merge, strategic, or json.",
            code="patch_type_invalid",
            payload={"patch_type": patch_type},
        )
    patch_body = target.get("patch_body")
    if patch_type == "json":
        if not isinstance(patch_body, list):
            raise ActionRequestValidationError(
                "JSON patch body must be a list of operations.", code="patch_body_invalid"
            )
        ...
        shape = {"body_shape": "object", "top_level_fields": sorted(str(key)[:120] for key in patch_body)}
    _assert_patch_body_size(patch_body)
    sanitized = sanitize_action_value(patch_body)
    if "[redacted]" in str(sanitized):
        raise ActionRequestValidationError(
            "Patch body contains sensitive data and cannot be stored in an action request.", code="patch_body_sensitive"
        )
...
def _assert_patch_body_size(value: Any) -> None:
    ...
    if encoded_size <= 0 or encoded_size > max_bytes:
        raise ActionRequestValidationError(
            "Patch body is outside the allowed size.", code="patch_body_size_invalid", payload={"max_bytes": max_bytes}
        )
