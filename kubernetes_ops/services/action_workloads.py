...
def workload_restart_preview(target: dict[str, Any], *, summary: str) -> tuple[K8sCluster, dict[str, Any], dict[str, Any]]:
    ...
    return cluster, normalized, _base_preview(summary, normalized, workload, expected=["workload rollout status", "pod readiness", "recent warning events"])


def workload_scale_preview(target: dict[str, Any], *, summary: str) -> tuple[K8sCluster, dict[str, Any], dict[str, Any]]:
    workload, cluster, namespace, name, kind = _workload_context(target)
    ...
    preview = _base_preview(summary, normalized, workload, expected=["desired replica count", "workload readiness", "recent warning events"])
...
def _workload_context(target: dict[str, Any]) -> tuple[K8sWorkloadRef | None, K8sCluster, str, str, str]:
    ...
    cluster = workload.cluster if workload is not None else _cluster_or_none(str(target.get("cluster_id") or target.get("cluster") or ""))
    ...
    kind = {"deploy": "deployment", "deployments": "deployment", "rs": "replicaset", "replicasets": "replicaset"}.get(kind, kind)
    if cluster is None:
    ...
    if not namespace or not name:
        raise ActionRequestValidationError("workload action requires namespace and name.", code="workload_target_required", payload={"target": target})
    if workload is None:
...
def _base_preview(summary: str, normalized: dict[str, Any], workload: K8sWorkloadRef | None, *, expected: list[str]) -> dict[str, Any]:
    return {
        "summary": summary,
...
def _clean_replicas(value: Any) -> int:
    try:
        ...
        raise ActionRequestValidationError("replicas must be an integer.", code="replicas_invalid", payload={"replicas": value}) from exc
    if replicas < 0 or replicas > int(getattr(settings, "KUBERNETES_ADMIN_SCALE_MAX_REPLICAS", 100)):
        raise ActionRequestValidationError("replicas is outside the allowed range.", code="replicas_out_of_range", payload={"replicas": replicas})
    return replicas
