...
def api_kubernetes_admin_node_debug_start(request, session_id):
    def handler():
        ...
        return JsonResponse({"success": True, "node_debug": envelope})
...
@require_feature("kubernetes")
@require_http_methods(["POST"])
def api_kubernetes_admin_node_debug_stop(request, session_id):
    def handler():
        data, error_response = _json_body(request)
        if error_response:
            return error_response
