...
def _safe_json(handler):
    try:
        ...
        logger.exception("kubernetes admin workload API failed: %s", exc)
...
def api_kubernetes_admin_scale(request, cluster_id: str):
    def handler():
        ...
        try:
            payload = scale_kubernetes_workload(
                ...
                api_version=str(data.get("api_version") or "apps/v1"),
...
@require_feature("kubernetes")
@require_http_methods(["POST"])
def api_kubernetes_admin_restart(request, cluster_id: str):
    def handler():
        data, error_response = _json_body(request)
        if error_response:
            return error_response
        try:
            payload = restart_kubernetes_workload(
                user=request.user,
                session_id=str(data.get("session_id") or ""),
                cluster_id=cluster_id,
                api_version=str(data.get("api_version") or "apps/v1"),
